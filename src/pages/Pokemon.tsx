...
function Pokemon() {
  ...
  const getPokemonInfo = useCallback(
    async () => {
      const { data } = await axios.get(`${singlePokemonRoute}/${params.id}`);
  ...
  useEffect(() => {
    ...
    // imageElemet.crossOrigin = "anonymous";
    // imageElemet.src = currentPokemon?.image;
    const downloadedImg = new Image();
    downloadedImg.crossOrigin = "anonymous";
    downloadedImg.src = currentPokemon?.image;
    const options = {
      pixels: 10000,
      distance: 1,
      splitPower: 10,
    ...
    const getColor = async () => {
      const color = await extractColors(downloadedImg.src, options);
  ...
  }, [params.id, getPokemonInfo,currentPokemon?.image]);
